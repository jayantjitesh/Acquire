...
import acquire.player.strategy.Strategy;
...
/**
 * A BasicPlayer is a new(String _name,Strategy _strategy,StateI _currentState)
 * Interp: _name : A String, defines the name of this player in the game
 * _strategy : A Strategy, defines the Strategy of this Player _currentState : A
 * StateI, defines the read only object of current game state
 */
...
public class BasicPlayer implements Player {
  ...
  public BasicPlayer(String name, Strategy strategy) {
    _name = name;
    _strategy = strategy;
  }
  ...
  private PossibleResult getMoveRequest(PlayerPerspective perspective) {
    Map<Location, PossibleResult> validMoves = getValidMoves(perspective);
   ...
   *         effects
   */
  protected static Map<Location, PossibleResult> getValidMoves(
      PlayerPerspective perspective) {
    Map<Location, PossibleResult> valid = new HashMap<Location, PossibleResult>();

    for (Location tile : perspective.getMyself().getTilesInHand()) {
      InspectResult result = perspective.getBoard().inspect(tile);
      if (result.isValid()) {
        valid.put(tile, (PossibleResult) result);
      }
    }
    return valid;
  }
